...
public class WebServiceConnectorTest extends TestCase {
    ...
    public WebServiceConnectorTest(String testName) {
        super(testName);
    }
    ...
    public static Test suite() {
        TestSuite suite = new TestSuite(WebServiceManagerTest.class);
    ...
    public void testWebServiceManager() throws Exception {
        ...
        WebServiceConnector.init(instance);
        // load the files
        instance.load(deploymentLoader);
        
        // retrieve the keys
        Set paths = WebServiceConnector.getInstance().getServices();
        if (paths.size() != 1) {
        ...
        if (WebServiceConnector.getInstance().getService("/WebServiceTest") 
                == null) {
            fail("Failed to retrieve the service reference.");
        }
