...
import com.rift.coad.script.server.files.php.PHPFileManagerImpl;
 ...
 * @author brett
 */
public class PHPFileEditor extends HttpServlet {

    // class singletons
    private static Logger log = Logger.getLogger(PHPFileEditor.class);
     ...
     */
    public PHPFileEditor() throws Exception {
        try {
            Configuration config = ConfigurationFactory.getInstance().getConfig(
                    PHPFileManagerImpl.class);
            base = new File(config.getString(PHPFileManagerImpl.PHP_BASE));
        } catch (Exception ex) {
    ...
    private void processGetFile(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (suffix.equals("property")) {
                ...
                request.setAttribute("type",
                        PHPFileSuffixLookup.getTypeForSuffix(
                        suffix));
            }

            RequestDispatcher requestDispatcher = getServletContext()
                .getRequestDispatcher("/editor/phpeditor.jsp");
