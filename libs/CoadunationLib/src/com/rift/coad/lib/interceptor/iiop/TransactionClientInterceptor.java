...
/**
 * This object initializes the JTA transaction so that it can be used by the
 * server.
 *
 * @author Brett Chaldecott
 */
public class TransactionClientInterceptor extends InterceptorWrapper implements
     ...
     */
    public TransactionClientInterceptor(ORBInitInfo info) {
    }
     ...
     */
    public String name() {
        return "TransactionClientInterceptor";
    }
    ...
    public void send_request(ClientRequestInfo ri) throws ForwardRequest {
        try {
            log.debug("Send a JTA client request");
            Current current = Current.getCurrent();
            TransactionContext txCtx = current.getPropagationContext(false);
            ServiceContext jtaContext = new ServiceContext(
                    Constants.JTA_CONTEXT,ObjectSerializer.
                    serialize(new JTAContextWrapper(
                    NamingDirector.getInstance().getInstanceId(),txCtx)));
            ri.add_request_service_context(jtaContext,true);
        } catch (Exception ex) {
            log.debug(
