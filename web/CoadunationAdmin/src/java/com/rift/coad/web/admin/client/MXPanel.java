...
/**
 * This panel provides a means to access and manage the mx beans.
 *
 * @author brett chaldecott
 */
public class MXPanel extends Composite implements ObjectsListener, 
    ...
    private MXManagerAsync service = null;
    ...
    public MXPanel() {
        ...
        service.getMXBeans(new AsyncCallback() {
     ...
     * init the daemon manager
     */
    private void initMXManager() {
        // Create the client proxy. Note that although you are creating the
        // service interface proper, you cast the result to the asynchronous
        // version of
        // the interface. The cast is always safe because the generated proxy
        // implements the asynchronous interface automatically.
        service = (MXManagerAsync) GWT.create(MXManager.class);
        // Specify the URL at which our service implementation is running.
        // Note that the target URL must reside on the same domain and port from
        // which the host page was served.
        //
        ServiceDefTarget endpoint = (ServiceDefTarget) service;
        String moduleRelativeURL = GWT.getModuleBaseURL() + "mxmanager";
    ...
    public void onClick(Widget widget) {
        if (help == widget) {
            HelpDialog.getInstance().setContent(
                    "The MX Admin Panel enables users to interact directly with" +
                    "<br>the deployed MX Beans. This means it is possible to" +
                    "<br>manage the beans without deploying a custom frontend." +
                    "<br>" +
                    "<br>To interact with a deployed MX Bean follow these steps." +
                    "<br>" +
                    "<br><b>Step 1:</b>" +
                    "<br>Select the bean to interact with by selecting the bean" +
