...
for (SPARQLResultRow row : entries) {
    ...
    if (domainName.equals(currentDomain)) {
        List<Resource> accountsResource = domainResource.listProperties("http://www.coadunation.net/schema/rdf/1.0/mail#Account")
        for (Resource accountResource : accountsResource) {
            IPService service = accountResource.get(IPService.class)
            domainResource.removeProperty("http://www.coadunation.net/schema/rdf/1.0/mail#Account",accountResource)
