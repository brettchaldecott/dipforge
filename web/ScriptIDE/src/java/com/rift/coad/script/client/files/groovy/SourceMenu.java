...
public class SourceMenu extends Menu implements ItemClickHandler {
    ...
    public class DeleteHandler implements AsyncCallback {
        ...
        public void onSuccess(Object result) {
            String id = "groovy." + scriptInfo.getScope() + "." + scriptInfo.getFileName();
            try {
    ...
    public SourceMenu(RDFScriptInfo scriptInfo, FileEditorFactory factory) {
        ...
        if (GroovyFileSuffixLookup.isFileViewable(scriptInfo.getFileName())) {
            MenuItem viewEntry = new MenuItem("View");
            items.add(viewEntry);
        }
    ...
    public void onItemClick(ItemClickEvent event) {
        if (event.getItem().getTitle().equalsIgnoreCase("Delete")) {
            GroovyFileManagerConnector.getService().deleteFile(scriptInfo.getScope(),
                    scriptInfo.getFileName(), new DeleteHandler(factory));
        } else if (event.getItem().getTitle().equalsIgnoreCase("View")) {
            new ViewPageWindowFactory(factory,scriptInfo.getScope(),
                    scriptInfo.getFileName()).createWindow();
        }
    }
